...
class SideBarItems extends Component {
    ...
    uniqueFilter = async (property, value, value2) => {
        const { itemsReducer, setItemsFilters, viewOne, itemsReducer: { location, searchByCategories }, setNextPage } = this.props;
    ...
    changePrice = async (event) => {
        this.setState({
            PriceValue: event.nativeEvent.target.value
        });
    };
    subCategorySearch = async (category, subC1 = '', subC2 = '', subC3 = '') => {
        const { setItemsFilters, setIdCategory, setLocation, itemsReducer: { searchByCategories, location }, setNextPage, setSearchCategoryObj } = this.props;
                ...
                <div>
                    ...
                    <div>
                        <button
                            ...
                            style={{ textDecoration: 'none', fontWeight: 'bold', color: 'rgb(0, 96, 234)', cursor: 'pointer' }}
                        ...
                        <>
                            ...
                            <ul>
                                            ...
                                            {category1.length > 0 && category1.map((subCategory1, subC1Index) => (
                                                <li>
                                                    {subCategory1.category === categoria.U_Categoria && subCategory1.subC1.map((subC1) => (
                                                        <div >
                                                            <div className='category' style={{ cursor: 'pointer' }} onClick={() => this.subCategorySearch(categoria.U_Categoria, subC1.U_Subcategoria1)}>
                                                                ...
                                                                {category2.length > 0 && category2.map((subCategory2, subC2Index) => (
                                                                    <li>
                                                                        {subCategory2.category === subCategory1.category && subCategory2.Subcategory1 === subC1.U_Subcategoria1 && subCategory2.subC2.map((subC2) => (
                                                                            <div>
                                                                                <div className='category' style={{ cursor: 'pointer' }} onClick={() => this.subCategorySearch(categoria.U_Categoria, subC1.U_Subcategoria1, subC2.U_Subcategoria2)}>
                                                                                    ...
                                                                                    {category3.length > 0 && category3.map((subCategory3, subC3Index) => (
                                                                                        <li>
                                                                                            {subCategory3.category === subCategory2.category && subCategory3.Subcategory1 === subCategory2.Subcategory1 && subCategory3.Subcategory2 === subC2.U_Subcategoria2 && subCategory3.subC3.map((subC3) => (
                                                                                                <div className='category' style={{ cursor: 'pointer' }} onClick={() => this.subCategorySearch(categoria.U_Categoria, subC1.U_Subcategoria1, subC2.U_Subcategoria2, subC3.U_Subcategoria3)}>
                                ...
                                {categoryShow < category.length && (
                                    <label
                                        style={{ color: "rgb(0, 96, 234)", cursor: 'pointer', margin: '0 10px'}}
                                        onClick={() => this.verMas('category', 7)} // Agrega el número 7
                                    >
                                ...
                                {categoryShow >= 7 && (
                                    <label
                                        style={{ color: "rgb(0, 96, 234)", cursor: 'pointer', margin: '0 10px' }}
                                        onClick={() => this.verMenos('category', 7)} // Agrega el número 7
                                    >
                    ...
                    {(!showCopy && dataCategories && dataCategories.itemsBrands.length > 0) &&
                        <ul className="mt-2 SubCategorias">
                            <dl>
                                {seleccionados.length > 0 && (
                                    <label style={{ color: "rgb(0, 96, 234)", cursor: 'pointer' }} onClick={() => this.DeleteFilter('MARCAS')}>Borrar filtro Marcas</label>
                                )}
                                {dataCategories.itemsBrands.slice(0, cantidadMostrada).map((brand, index) => {
                                    return (
                                        <>
                                            <div className='align-items-center' key={index}>
                                                ...
                                                {/* <dd className = 'linkFilter' onClick = {() => this.uniqueFilter('marca', brand.code, '')}>
                                                {brand.brand} 
                                            </dd> */}
                                            </div>
                                ...
                                {cantidadMostrada < dataCategories.itemsBrands.length ? (
                                    <>
                                        <label
                                            style={{ color: "rgb(0, 96, 234)", cursor: 'pointer', margin: '0 10px'  }}
                                            onClick={() => this.verMas('brand')}
                                        >
                                            <strong> Ver más</strong>
                                        </label>
                                        {cantidadMostrada >= 20 && (
                                            <label
                                                style={{ color: "rgb(0, 96, 234)", cursor: 'pointer', margin: '0 10px' }}
                                                onClick={() => this.verMenos('brand', 20)}
                                            >
                                                <strong> Ver menos</strong>
                                            </label>
                                        )}
                        ...
                        <ul className="mt-2 SubCategorias">
                            <dl>
                                ...
                                {cantidadMostrada < brandsCopy.length && (
                                    <label style={{ color: "rgb(0, 96, 234)", cursor: 'pointer' }} onClick={() => this.verMas('brand')}><strong> Ver más</strong></label>
                                )}
                                {cantidadMostrada > 10 && (
                                    <label style={{ color: "rgb(0, 96, 234)", cursor: 'pointer' }} onClick={() => this.verMenos('brand')}><strong>Ver menos</strong></label>
                                )}
                            </dl>
                        </ul>
                    }
                            ...
                            <dl>
                                <div class="input-group mb-3 pr-5">
                                    ...
                                    <input
                                        ...
                                        onKeyDown={event => PriceValue != 0 || PriceValue != '' ? (event.keyCode === 13) && this.uniqueFilter('precio', Number(PriceValue), '') : ""}
                                        onKeyPress={(event) => (event.charCode >= 48 && event.charCode <= 57) ? "" : event.preventDefault()}
                                        value={PriceValue}
                                    />
                                    <div class="input-group-append d-md-none">
                                        <span class="input-group-text">
                                            <i class="fa fa-search mr-1 ml-1" onClick={PriceValue !== 0 || PriceValue !== '' ? () => this.uniqueFilter('precio', Number(PriceValue), '') : ""} aria-hidden="true" style={{ color: "black", cursor: "pointer" }} />
                                        </span>
                                    </div>
                                </div>
                            </dl>
